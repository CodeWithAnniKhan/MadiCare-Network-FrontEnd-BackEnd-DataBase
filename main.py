...
@app.route('/department/cardiology')
def cardiology():
    return render_template('department/cardiology.html')

@app.route('/department/dermatology')
def dermatology():
    return render_template('department/dermatology.html')

@app.route('/department/neorology')
def neorology():
    return render_template('department/neorology.html')

@app.route('/department/nephrology')
def nephrology():
    return render_template('department/nephrology.html')

@app.route('/department/orthopad')
def orthopad():
    return render_template('department/orthopad.html')

@app.route('/department/psychology')
def psychology():
    return render_template('department/psychology.html')
...
def signup():
    if request.method == 'POST':
        try:
            ...
            session['user_id'] = name  # Store the user's name in the session
...
class Booking(db.Model):
    ...
    number = db.Column(db.String(15), nullable=False, server_default=text("''"))
    email = db.Column(db.String(30), nullable=False)
...
def booking():
    if request.method == 'POST':
        try:
            ...
            entry = Booking(name=name, doctor=doctor,department=department, number=number, email=email, address=address, problem=problem, date=datetime.now())
            db.session.add(entry)
            db.session.commit()
        except Exception as e:
            # Handle the exception, print or log the error for debugging
